...
- **Ein `main()` in `scripts/` ist nur durch den Spawn-Sweep in `tests/cli.test.mjs` gedeckt** (jedes Script einmal bis zu seiner eigenen ersten Validierungsmeldung, ohne Netz). Neues Script oder neuer Flag-Pfad → dort eine Zeile, sonst bleibt die Suite grün, während der Cron-Job vor seinem Push-Step stirbt (2026-09-10: zweimal, nach C4).
- **Node-Scripts mit Netzwerk laufen am Sandbox-Proxy vorbei:** undici/`fetch` kennt `HTTP_PROXY` nicht → `ENOTFOUND www.pegelonline.wsv.de`, obwohl `curl` denselben Host erreicht. Das ist die Sandbox, nicht DNS und nicht die App — ein Bypass pro Call statt Debugging, ohne sandboxed Erstversuch (betrifft `scripts/fetch-wsv-archive.mjs`, Ad-hoc-Node gegen die WSV-APIs und jeden `LANUK_BASE_URL=`/`GATE_BASE_URL=`-Lauf der `verify-*.mjs` gegen die Live-Seite — dort `ENOTFOUND bmmmm.github.io`).
- **`archive`-Branch = GitHub-only Orphan-Datenbranch.** Pushes dorthin triggern nie einen Workflow (kein `.github/` im gepushten Commit) — Deploys brauchen den expliziten `gh workflow run pages.yml --ref main` — **nur nach Datenbranch-Pushes**: ein Push auf main triggert Pages selbst, und ein zusätzlicher Dispatch kollidiert damit (2026-09-10: ein Deploy lief GitHub-seitig in ein Timeout, jeder Folgelauf scheiterte mit „in progress deployment", der Cancel-Endpoint antwortete `deployment_cancelled` und die Sperre hielt ~25 min — warten, keine weiteren Runs); das Reseed-Runbook steht im Header von `scripts/fetch-wsv-archive.mjs`.
