# Domains — the knowledge that is not needed on every turn

`CLAUDE.md` is re-read into every session's context, so what lives there has to
earn its keep on every turn. These files carry the rest: still canonical, still
verbatim, just loaded on demand. Read the one that covers what you are about to
touch — and inject it into a subagent's prompt when you delegate that work.
